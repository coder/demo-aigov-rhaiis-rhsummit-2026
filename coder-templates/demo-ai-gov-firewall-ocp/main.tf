  ...
  spec {
    container {
      ...
      security_context {
        ...
        # firewall process. The workspace SA in `coder-workspaces`
        # must be bound to a SCC that permits these caps — see
        # README and the SCC ClusterRoleBinding shipped via
        # `gitops/apps/coder` (or applied imperatively at bootstrap).
        # run_as_user is intentionally omitted: OCP's SCC mutating
        # admission injects a UID from the namespace's uid-range.
        capabilities {
